  ...
  useEffect(() => {
    ...
    const handleError = (error: any) => {
      ...
      if (Object.keys(error).length === 0) {
        console.error(
          "The error object is empty. This often happens if the Public Key is invalid or the connection was blocked.",
        );
      }
      setConnecting(false);
      setCallActive(false);
  ...
  const toggleCall = async () => {
    if (callActive) vapi.stop();
      ...
      try {
        ...
        if (!assistantId) {
          throw new Error(
            "NEXT_PUBLIC_VAPI_ASSISTANT_ID is missing from environment variables.",
          );
        }
        ...
        console.error(
          "Failed to start call catch block:",
          error.message || error,
        );
        setConnecting(false);
      }
